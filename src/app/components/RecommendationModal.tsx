...
import { X, ArrowRight, CheckCircle2, TrendingUp, Wifi, Timer, AlertTriangle, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { toast } from 'sonner';
import { AccessPoint, Config } from '../types/wifi';
import { WifiMonitor, isNative } from '../services/nativeWifi';
import { scheduleAdaptiveWeightCheck } from '../services/adaptiveWeights';
...
export function RecommendationModal({
  ...
  const handleSwitch = async () => {
    if (!isNative()) {
      toast.info('Switching access points requires the Android app.');
      return;
    }

    setIsSwitching(true);
    try {
      const result = await WifiMonitor.switchToBssid({ ssid: recommendedAP.ssid, bssid: recommendedAP.bssid });
      if (result.success) {
        scheduleAdaptiveWeightCheck(currentAP, recommendedAP);
        onClose();

        toast.loading(`Switched to ${recommendedAP.ssid} — checking internet...`, { id: 'download-check' });
        try {
          const downloadResult = await WifiMonitor.testDownload();
          if (downloadResult.success) {
            toast.success(`Switched to ${recommendedAP.ssid} — internet confirmed`, { id: 'download-check' });
          } else {
            toast.error(`Switched to ${recommendedAP.ssid}, but this AP has no working internet`, { id: 'download-check' });
          }
        } catch (downloadError) {
          toast.success(`Switched to ${recommendedAP.ssid}`, { id: 'download-check' });
        }
      } else {
        toast.error('Could not switch to that access point.');
      }
    } catch (error) {
            ...
            >
              ...
              <div className="p-6 space-y-6">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors disabled:opacity-60"
                >
                  {isSwitching ? <Loader2 size={18} className="animate-spin" /> : <ArrowRight size={18} />}
                  {isSwitching ? 'Switching...' : `Switch to ${recommendedAP.ssid}`}
                </button>
